...
class ModelParameters:
    ...
    def get_model(self):
        ...
        extractor_flow = ResnetFeatureExtractor(name='resnet18').to(self.device)
        ...
        mdl = TrainerModel(augmenter, extractor, extractor_flow, self.disc_params, seg_network, batch_size=self.batch_size,
...
if __name__ == '__main__':
    ...
    trainer = Trainer(args.name, model, optimizer=optimizer, scheduler=scheduler, dataset=dataset, checkpoints_path=paths['checkpoints'],
                      log_path=paths['tensorboard'], max_epochs=260, batch_size=params.batch_size, num_workers=4, load_latest=True, save_interval=10)
    trainer.train()
