...
from .raft_utils import coords_grid, CorrBlock, upflow8
import cv2
...
def save_tensor(input):
    ...
    heatmap = cv2.applyColorMap(np.uint8(255 * input), cv2.COLORMAP_JET)
...
class RAFT(nn.Module):
    ...
    def forward(self, fmap1, fmap2, image, iters=3, flow_init=None, upsample=True, test_mode=False):

        net = torch.tanh(fmap1)
        corr_fn = CorrBlock(fmap1, fmap2, radius=4)
        coords0, coords1 = self.initialize_flow(image)
        ...
        for itr in range(iters):
            ...
            with autocast(enabled=True):
                net, up_mask, delta_flow = self.update_block(net, corr, flow)
